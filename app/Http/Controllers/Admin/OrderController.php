    ...
    public function getList($status, $type)
    {
        if($status == 'all'):
            if($type == 'all'):
                ...
                $orders = Order::where('status', '!=', '0')->where('o_type', $type)->with(['getUser'])->orderBy('o_number', 'Desc')->paginate(25);
            endif;
        else:
            if($type == 'all'):
                $orders = Order::where('status', $status)->where('o_type', $type)->with(['getUser'])->orderBy('o_number', 'Desc')->paginate(25);
            else:
                $orders = Order::where('status', $status)->where('o_type', $type)->with(['getUser'])->orderBy('o_number', 'Desc')->paginate(25);
            endif;
        endif;
